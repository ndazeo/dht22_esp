/* Multi-client Server Sent Event (aka EventSource) demo
  ...
  2. on the client(s), register it for the event bus using a REST API call: curl -sS "http://<your ESP IP>:<your port>/events/sensors"
...
#ifndef STASSID
#define STASSID "Casita"
#define STAPSK  "Piedritas"
#endif
...
void SSEBroadcastState(const char *sensorName, float prevSensorValue, float sensorValue) {
  for (uint8_t i = 0; i < SSE_MAX_CHANNELS; i++) {
    if (!(subscription[i].clientIP)) {
      continue;
    }
    ...
    if (subscription[i].client.connected()) {
      ...
      subscription[i].client.printf_P(PSTR("event: update\ndata: {\"TYPE\":\"STATE\", \"%s\":{\"state\":%f, \"prevState\":%f}}\n\n"),
...
void updateSensor(sensorType &sensor) {
  const float newVal = sensor.type == temperature ? dht.readTemperature(0) : dht.readHumidity();
  Serial.printf_P(PSTR("update sensor %s - previous state: %f, new state: %f\n"), sensor.name, sensor.value, newVal);
  if (sensor.value != newVal) {
    //SSEBroadcastState(sensor.name, sensor.value, newVal);  // only broadcast if state is different
  }
  sensor.value = newVal;
  sensor.update.once(15, std::bind(updateSensor, sensor));  // update sensor every 15 seconds
}
...
void startServers() {
  server.on(F("/events/sensors"), handleSubscribe);
...
void setup(void) {
  ...
  sensor[0].name = "Temperature";
  sensor[0].type = temperature;
  sensor[1].name = "Humidity";
  sensor[1].type = humidity;
